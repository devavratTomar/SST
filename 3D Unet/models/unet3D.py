...
def getNormLayer(och, norm="batch"):
    if norm == "batch":
        norm_layer = nn.BatchNorm3d(och, affine=False)
    elif norm == "inst":
        norm_layer = nn.InstanceNorm3d(och, track_running_stats=False)
    else:
        norm_layer = None
    return norm_layer
...
class Double_conv(nn.Module):
    ...
    def __init__(self, ich, och, norm="batch"):
        ...
        """
        self.conv = nn.Sequential(nn.Conv3d(ich, och, 3, 1, 1, bias=False),
                                  norm_layer,
                                  nn.LeakyReLU(0.2),
                                  nn.Conv3d(och, och, 3, 1, 1, bias=False),
                                  norm_layer,
                                  nn.LeakyReLU(0.2))
        
        """
        self.conv = nn.Sequential(nn.Conv3d(ich, och, 3, 1, 1),
...
class Down(nn.Module):
    ...
    def __init__(self, ich, och, downsample=False, norm="batch"):
        ...
        if downsample:
            ...
            self.pool = nn.Conv3d(ich, ich, 4, 2, 1, bias=False)
...
class Up(nn.Module):
    ...
    def __init__(self, ich, och, norm="batch", divide_nch_by=2):
        super(Up, self).__init__()

        norm_layer = getNormLayer(ich // divide_nch_by, norm)
        """
        self.up = nn.Sequential(nn.Upsample(mode='trilinear', scale_factor=2, align_corners=True),
                                nn.Conv3d(ich, ich // divide_nch_by, kernel_size=1, bias=False),
                                norm_layer)
        """
        self.up = nn.Sequential(nn.Upsample(mode='trilinear', scale_factor=2, align_corners=True),
                                nn.Conv3d(ich, ich // divide_nch_by, kernel_size=1, bias=False))

        self.conv = Double_conv(och + ich // divide_nch_by, och, norm=norm)
...
class UNET3D(nn.Module):
    ...
    def __init__(self, ich, n_classes, downsample=False, norm="batch"):
        ...
        self.final = nn.Conv3d(32, n_classes, 1, 1, 0, bias=False)
